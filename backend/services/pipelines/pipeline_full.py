"""
Pipeline "full" : ElevenLabs TTS + videos_db + overlays bibliques.
Équivalent de video_gen_full.py (réécrit pour fonctionner sans working_dir fixe).
"""
...
def run_pipeline_full(
    ...
    TTS → SRT → détection prières → versets → vidéo fond → mix → encodage final.
    Retourne le chemin de la vidéo finale.
    """
    log("🚀 Pipeline FULL démarré", log_file)
    ...
    # Sauvegarder le texte nettoyé (utilisé pour la détection des versets)
    ...
    log("\n🎬 Étape 6/7 : Assemblage de la vidéo de fond...", log_file)
